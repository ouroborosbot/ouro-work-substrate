...
describe("Deploy Azure workflow production SMTP port contract", () => {
  ...
  it("keeps outbound ACS email resources and Event Grid delivery reports in the deployable Azure shape", () => {
    ...
    expect(bicep).toContain("param outboundEmailDomainExists bool = false")
    expect(bicep).toContain("outboundEmailLinkVerifiedDomain bool = false")
    expect(bicep).toContain("resource outboundEmailDomainExisting")
    expect(bicep).toContain("var outboundEmailDomainId = outboundEmailDomainExists ? outboundEmailDomainExisting!.id : outboundEmailDomain!.id")
    ...
    expect(workflow).toContain("outbound_email_domain_exists")
    expect(workflow).toContain("outbound_email_link_verified_domain")
    expect(workflow).toContain(".Domain.status")
    expect(workflow).toContain(".SPF.status")
    expect(workflow).toContain(".DKIM.status")
    expect(workflow).toContain(".DKIM2.status")
    expect(workflow).toContain("Domain resource already exists")
    expect(workflow).toContain("outboundEmailDomainExists=")
    ...
    expect(operations).toContain("treats an existing ACS custom domain as an existing resource")
