...
- 2026-04-23 09:28 Live native autonomy proof exposed a production rate-limit hole: provider-backed ACS sends in `submitted` state were not counted yet, so two autonomous sends could slip through before delivery events arrived. Harness branch `slugger/mail-autonomy-rate-limit` implements the fix and its live-proof checkpoint is recorded in `unit6-rate-limit-release-checkpoint.md`.
- 2026-04-23 10:12 Delegated HEY live proof succeeded for the `arimendelow@hey.com` feeder: forwarding confirmation landed in the delegated alias, a live mirror proof arrived in the delegated lane with correct provenance, and the hosted large-MBOX import path was proven against a real 2.4 GiB HEY export. Evidence lives in `unit5-live-hey-proof.md`.
- 2026-04-23 10:35 Rebased the autonomy fix branch onto current main at `001f1494`, reran full harness coverage/preflight at 100%, and pushed PR #596 so the release lane can merge/publish the fix. At this checkpoint `integration` and `package-e2e` were green while `coverage` was still running.
- 2026-04-23 10:43 Committed and pushed harness branch `slugger/mail-import-hosted` with hosted-registry reader support, streaming large-MBOX import, resumable/idempotent hosted import behavior, and doc updates that teach linked HEY accounts as separate export/forwarding feeders. The branch still needs post-`.475` rebump, release validation, PR, merge, publish, and install.
