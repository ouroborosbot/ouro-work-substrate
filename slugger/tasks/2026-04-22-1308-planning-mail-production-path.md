...
- Add DNS provider automation and runbooks: credential-item binding, current-record backup, dry-run, apply, verification, rollback, and audit notes for A/MX/TXT/CNAME changes. Porkbun is the current provider driver for `ouro.bot`, not the credential model.
...
- Add recovery handling for every brittle setup edge: missing DNS provider access, DNS propagation, port-25 exposure failure, Mail Control failure, vault locked/missing, one-time key loss, registry/key drift, HEY forwarding failure, missing forwarded mail, Blob access failure, decryption failure, wrong placement, outbound provider failure, and delivery events.
...
- Human approval/process gates are waived for this task. Remaining gates are genuine external inputs: DNS provider API access, secrets/vault unlock, browser/MFA steps for HEY, and any provider/portal confirmation that cannot be completed through current CLI/API access.
...
- Porkbun is the current DNS provider for `ouro.bot`. Use it as the first DNS driver for backup/dry-run/apply/rollback once credentials are safely provisioned, but keep the credential model generic: ordinary agent-vault item first, workflow binding second, provider driver/template last.
...
- Human-needed lock list:
  - DNS provider API: human creates or approves the provider credential needed for `ouro.bot`, does not paste it into chat, and enters it only through a hidden prompt or approved secret store. The current provider is Porkbun. Needed for DNS backup/dry-run/apply/rollback and certificate automation.
  - Slugger vault/secret entry: human may need to unlock Slugger's vault or enter secret material through a hidden prompt when setup stores Mailroom private keys, hosted Blob reader config, DNS provider credentials, ACS credentials, or outbound/autonomy policy secrets.
