...
- 2026-04-22 19:13 Completed the harness vault-item release lane: PR #587 merged, main publish passed, npm/latest is `0.1.0-alpha.466`, local `ouro` is alpha.466, and Slugger's Porkbun item is visible through the generic vault item surface without printing secrets.
- 2026-04-22 19:18 Added two-lane red tests across the hosted protocol and harness so native agent mail and delegated human mailbox access cannot collapse silently in provenance, access logs, attention summaries, outbound records, or Outlook source folders.
- 2026-04-23 09:28 Live autonomy proof refined the production bar again: provider-backed `submitted` outbound sends must count immediately against the autonomous rate window, not only later `sent` records after delivery-event reconciliation.
- 2026-04-23 10:12 Delegated HEY proof clarified the user-facing design: keep one Ari delegated HEY source lens, but treat linked HEY accounts as separate operational feeders for export and forwarding. `arimendelow@hey.com` is live-proven; `ari@mendelow.me` still needs its own password-gated export and forwarding confirmation.
- 2026-04-23 10:43 Hosted HEY archive import is now a first-class production requirement, not a local convenience: the harness must read hosted registry coordinates from the vault, stream large MBOX files into Blob-backed Mailroom storage, and remain resumable/idempotent on rerun after interruption.
